...
# Authenticated client for user_a
@pytest.fixture
def auth_a(user_a):
    client = APIClient()
...
# Authenticated client for user_b
@pytest.fixture
def auth_b(user_b):
    client = APIClient()
...
def test_tasks_list_only_for_board_members(auth_a, auth_b, board, user_a):
...
@pytest.mark.django_db
def test_task_detail_returns_404_if_not_exists(auth_a):
...
@pytest.mark.django_db
def test_create_task_forbidden_if_not_board_member(auth_b, board):
    res = auth_b.post(
        "/api/tasks/",
...
@pytest.mark.django_db
def test_patch_task_invalid_status_returns_400(auth_a, board, user_a):
...
@pytest.mark.django_db
def test_delete_task_only_creator_or_owner(auth_a, auth_b, board, user_a, user_b):
...
@pytest.mark.django_db
def test_assigned_to_me_filter(auth_a, board, user_a, user_b):
    board.members.add(user_b)
...
def test_reviewing_filter(auth_a, board, user_a):
    Task.objects.create(board=board, title="Review Task", created_by=user_a, reviewer=user_a)
